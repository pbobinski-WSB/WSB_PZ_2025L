...
import wsb.merito.pz.cw03.klasy.ProduktWkoszyku;
...
public class Sklep implements Sklepable {
    ...
    public void platnos() {
        ...
        System.out.println("Kolejka "+kolejka.stream().map(k -> new NameCartCount(k.getName(), k.getKoszyk().getCartCount())).toList());
        Klient klient = null;
        while ((klient = kolejka.poll()) != null) {
            ...
            System.out.println("Kolejka "+kolejka.stream().map(k -> k.getName()).collect(Collectors.toList()));
        }

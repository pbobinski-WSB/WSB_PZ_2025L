package wsb.merito.pz.cw08.serialization;
...
public class Main {
    ...
    private static void graphSerialisation() throws IOException, ClassNotFoundException {
        ...
        try (ObjectOutputStream outputStream = new ObjectOutputStream(new FileOutputStream("file/object-graph.bin"))) {
            outputStream.writeObject(serializedCar);
        }

        Car deserializedCar = null;
        try (ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream("file/object-graph.bin"))) {
    ...
    private static void simpleSerialization() throws IOException, ClassNotFoundException {
        try (ObjectOutputStream outputStream = new ObjectOutputStream(new FileOutputStream("file/objects.bin"))) {
            outputStream.writeObject(Integer.valueOf(1));
            outputStream.writeObject(Integer.valueOf(2));
        }

        try (ObjectInputStream inputStream = new ObjectInputStream(new FileInputStream("file/objects.bin"))) {
